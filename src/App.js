...
// --- FORMULAIRES ---
// (Les formulaires restent identiques, je les compacte légèrement pour la lisibilité)
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <Card className="p-10 w-full max-w-md shadow-2xl border-0 ring-1 ring-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full pl-4 pr-4 py-3.5 bg-slate-50 border-0 rounded-xl text-sm font-medium" placeholder="Identifiant" />
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full pl-4 pr-4 py-3.5 bg-slate-50 border-0 rounded-xl text-sm font-medium" placeholder="Mot de passe" />
...
const ClientReportForm = ({ nest, onSave, onCancel }) => {
  ...
  return (
    <div className="space-y-4">
        ...
        <textarea className="w-full p-2 border rounded-lg" value={formData.description} onChange={e=>setFormData({...formData, description: e.target.value})} placeholder="Description" />
        <div className="flex gap-2"><Button variant="outline" onClick={onCancel}>Annuler</Button><Button variant="sky" onClick={()=>onSave(formData)}>Envoyer</Button></div>
    </div>
  );
};
...
const LeafletMap = ({ markers, isAddingMode, onMapClick, onMarkerClick, center, routePath }) => {
  ...
  // REFS pour éviter les problèmes de closure dans les events
  ...
  // 2. Gestion Invalid Size lors du changement de mode (CRUCIAL POUR LE BUG BLANC)
  useEffect(() => {
...
// (Ces composants restent identiques à la version précédente fonctionnelle)
        ...
        <div className="space-y-8 text-slate-800">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="space-y-8">
                    <Card className="p-8 border-0 shadow-xl rounded-3xl bg-white">{isEditing ? <ClientEditForm client={selectedClient} onSave={(d) => {onUpdateClient(d); setIsEditing(false);}} onCancel={() => setIsEditing(false)}/> : (<><h2 className="text-2xl font-black mb-6 uppercase tracking-tighter text-slate-900">{selectedClient.name}</h2><div className="space-y-6 text-sm font-bold text-slate-600 uppercase"><div className="flex items-start gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100"><MapPin size={20} className="text-sky-500 shrink-0"/><p className="leading-tight text-xs">{selectedClient.address}</p></div><div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100"><Phone size={20} className="text-sky-500 shrink-0"/><p className="text-xs">{selectedClient.phone}</p></div><div className="bg-slate-900 p-6 rounded-2xl text-white shadow-lg"><p className="text-[10px] font-black opacity-50 tracking-widest text-center mb-4">ACCÈS ESPACE CLIENT</p><p className="text-xs tracking-widest mb-2"><span className="opacity-50">ID:</span> {selectedClient.username}</p><p className="text-xs tracking-widest"><span className="opacity-50">PASS:</span> {selectedClient.password}</p></div></div><div className="flex flex-col gap-3 mt-8"><Button variant="sky" className="w-full py-3 rounded-2xl uppercase tracking-widest text-xs font-black h-12" onClick={() => setIsEditing(true)}>Modifier</Button><Button variant="danger" className="w-full py-3 rounded-2xl uppercase tracking-widest text-xs font-black h-12" onClick={() => {if(window.confirm("Supprimer ce client ?")){onDeleteClient(selectedClient); setView("clients");}}}>Supprimer</Button></div></>)}</Card>
                </div>
                <div className="lg:col-span-2 space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><Card className="p-6 shadow-lg border-0 rounded-3xl bg-slate-900 text-white"><p className="text-[10px] font-black opacity-50 uppercase tracking-widest mb-2 text-center">Nids recensés</p><p className="text-5xl font-black text-sky-400 text-center">{cNests.length}</p></Card><Card className="p-6 shadow-lg border-0 rounded-3xl bg-sky-600 text-white"><p className="text-[10px] font-black opacity-50 uppercase tracking-widest mb-2 text-center">Missions effectuées</p><p className="text-5xl font-black text-white text-center">{cInt.filter(i => i.status === "Terminé").length}</p></Card></div>
                    <Card className="p-8 border-0 shadow-xl rounded-3xl bg-white"><h3 className="text-xl font-black uppercase tracking-tighter mb-6 text-slate-900">HISTORIQUE DES PASSAGES</h3><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-[10px] font-black uppercase tracking-widest border-b border-slate-100 text-slate-500"><tr><th className="p-4">Date</th><th className="p-4">Statut</th><th className="p-4">Notes</th></tr></thead><tbody className="divide-y divide-slate-100">{cInt.length === 0 ? <tr><td colSpan="3" className="p-8 text-center text-slate-400 font-bold uppercase text-xs italic">Aucune intervention</td></tr> : cInt.map(i => <tr key={i.id} className="hover:bg-slate-50 transition-colors"><td className="p-4 font-black text-slate-700">{i.date}</td><td className="p-4"><Badge status={i.status}/></td><td className="p-4 text-xs font-bold text-slate-500 italic truncate max-w-[200px]">{i.notes}</td></tr>)}</tbody></table></div></Card>
                </div>
            </div>
        </div>
    );
